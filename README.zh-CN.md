...
2. 运行 `OpenClaw-Setup-0.2.18.exe`
...
- **文档与发布策略：** 说明 **固定 OpenClaw 版本** 的方式（根目录 [`package.json`](package.json) 中 `openclawBundleVersion`，当前为 `2026.3.23-2`）；GitHub **Release CI** 设置 `OPENCLAW_SKIP_NPM_LATEST_CHECK=1`，使 `check-openclaw-versions` 只校验 **pin + manifest + 磁盘产物**，**不**与 npm `latest` 对比（上游发新版时减少流水线误报）。
...
从[最新发布页](https://github.com/agentkernel/openclaw-desktop/releases/latest)下载 `OpenClaw-Setup-0.2.18.exe` 并运行即可。无需 `npm`、无需系统级 Node.js、无需输入任何终端命令。
</details>
...
**捆绑 OpenClaw：** 在 `package.json` 中通过 `openclawBundleVersion` 固定；执行 `prepare-bundle` 后查看 [`resources/bundle-manifest.json`](resources/bundle-manifest.json) 中的 `bundledOpenClawVersion`（桌面 **v0.2.18** 当前为 **2026.3.23-2**）。本地校验：`pnpm run check-openclaw-versions`（不设 `OPENCLAW_SKIP_NPM_LATEST_CHECK` 时还会与 npm `latest` 对比）。
